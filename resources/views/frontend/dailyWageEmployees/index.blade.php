        ...
        <div class="col-md-12">
            @can('daily_wage_employee_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.daily-wage-employees.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $dailyWageEmployee->id }}">
                                        ...
                                        <td>
                                            @can('daily_wage_employee_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.daily-wage-employees.show', $dailyWageEmployee->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('daily_wage_employee_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.daily-wage-employees.edit', $dailyWageEmployee->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('daily_wage_employee_delete')
                                                <form action="{{ route('frontend.daily-wage-employees.destroy', $dailyWageEmployee->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('daily_wage_employee_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.daily-wage-employees.massDestroy') }}",
