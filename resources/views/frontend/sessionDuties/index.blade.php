        ...
        <div class="col-md-12">
            @can('session_duty_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.session-duties.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $sessionDuty->id }}">
                                        ...
                                        <td>
                                            @can('session_duty_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.session-duties.show', $sessionDuty->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('session_duty_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.session-duties.edit', $sessionDuty->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('session_duty_delete')
                                                <form action="{{ route('frontend.session-duties.destroy', $sessionDuty->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('session_duty_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.session-duties.massDestroy') }}",
