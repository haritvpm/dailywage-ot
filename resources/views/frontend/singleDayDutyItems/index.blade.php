        ...
        <div class="col-md-12">
            @can('single_day_duty_item_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.single-day-duty-items.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $singleDayDutyItem->id }}">
                                        ...
                                        <td>
                                            @can('single_day_duty_item_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.single-day-duty-items.show', $singleDayDutyItem->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('single_day_duty_item_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.single-day-duty-items.edit', $singleDayDutyItem->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('single_day_duty_item_delete')
                                                <form action="{{ route('frontend.single-day-duty-items.destroy', $singleDayDutyItem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('single_day_duty_item_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.single-day-duty-items.massDestroy') }}",
