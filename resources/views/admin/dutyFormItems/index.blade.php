...
@can('duty_form_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.duty-form-items.create') }}">
                        ...
                        <tr data-entry-id="{{ $dutyFormItem->id }}">
                            ...
                            <td>
                                @can('duty_form_item_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.duty-form-items.show', $dutyFormItem->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('duty_form_item_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.duty-form-items.edit', $dutyFormItem->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('duty_form_item_delete')
                                    <form action="{{ route('admin.duty-form-items.destroy', $dutyFormItem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('duty_form_item_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.duty-form-items.massDestroy') }}",
